(() => {
  ...
  const CACHEABLE_PATHS = new Set(["/api/v1/categories", "/api/v1/content/cms-content"]);
  ...
  async function cachedApiFetch(path, options = {}) {
    if (!CACHEABLE_PATHS.has(path) || options.body != null) {
  ...
  function initVacancies(signal) {
    ...
    if (!input) return;
    const tabs = $$(".scholarships__tab");
    const activeClass = "scholarships__tab--active";
    const cards = $$(".scholarship-card");
    const locale = "az";
    const filter = () => {
  ...
  function faqCard(template, item, fallback, index) {
    ...
    const title = $(".blog-card__title", card);
    const description = $(".blog-card__desc", card);
    const fallbackTitle = $(".blog-card__title", fallback)?.textContent.trim();
    ...
    card.href = `faq.html?target=${encodeURIComponent(key)}`;
  ...
  async function initFaqPage(signal) {
    const grid = $(".blogs");
    if (!grid) return;
    const currentCards = $$(".blog-card", grid);
    ...
    try {
      const content = await cachedApiFetch("/api/v1/content/cms-content", {
        signal,
      });
  ...
  function scholarshipCard(template, item, fallback, index) {
    const card = template.cloneNode(true);
    const title = $(".scholarship-card__title", card);
    const description = $(".scholarship-card__desc", card);
    const fallbackTitle = $(
  ...
  function refreshScholarshipFilters(signal) {
    const current = $(".scholarships__search");
    if (!current) return;
  ...
  async function initScholarshipsPage(signal) {
    const grid = $(".scholarships__grid");
    if (!grid) return;
    const currentCards = $$(".scholarship-card", grid);
    const template = currentCards[0];
    if (!template) return;
    const fallbackCards = currentCards.map((card) => card.cloneNode(true));
    grid.setAttribute("aria-busy", "true");
  ...
  function applyHomeBanner(root, banner) {
    const video = $(".hero-section__video", root);
    const data =
      banner?.data &&
  ...
  async function initHomeBanner(signal) {
    ...
    try {
      const content = await cachedApiFetch("/api/v1/content/cms-content", { signal });
      if (!Array.isArray(content))
  ...
  async function loadPublicCourseCatalog(signal) {
    ...
    const work = async () => {
      const categories = await cachedApiFetch("/api/v1/categories", { signal });
  ...
  async function initCategoryPage(signal) {
    ...
    try {
      const [category, categories] = await Promise.all([
        ...
        cachedApiFetch("/api/v1/categories", { signal }),
      ]);
      if (signal.aborted) return;
      const categoryState = publicCategoryState(categories);
      if (
